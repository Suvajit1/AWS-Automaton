...
def create_security_group_public(vpc_id):
    ...
    sg_id = ec2.create_security_group(
        ...
        Description="Allow SSH, HTTP, HTTPS, Pritunl",
        VpcId=vpc_id
    )["GroupId"]
    ...
    ec2.authorize_security_group_ingress(
        ...
        IpPermissions=[
            ...
            {"IpProtocol": "tcp", "FromPort": 443, "ToPort": 443,
             "IpRanges": [{"CidrIp": "0.0.0.0/0"}]},
            {"IpProtocol": "udp", "FromPort": 3559, "ToPort": 3559,
             "IpRanges": [{"CidrIp": "0.0.0.0/0"}]},
        ]
    )
...
def create_security_group_private(vpc_id):
    ...
    print(f"Private SG created: {sg_id}")
    return sg_id
...
curl -fsSL https://raw.githubusercontent.com/pritunl/pritunl-zero/master/scripts/install.sh | sudo bash
...
def launch_private_instance(subnet_id, sg_id, name_tag, key_name):
    print(f"Launching private instance: {name_tag}...")
    ...
    instance_id = instance["Instances"][0]["InstanceId"]
    print(f"Private Instance launched: {instance_id}")
    return instance_id
...
def main():
    ...
    private_sg = create_security_group_private(vpc_id)

    public_key = create_key_pair(PUBLIC_KEY_NAME)
    private_key = create_key_pair(PRIVATE_KEY_NAME)

    public_instance_id = launch_public_instance(subnets["public"][0], public_sg, "Pritunl-Public-Instance", public_key)
    # Save public instance ID to file
    with open("public_instance_id.txt", "w") as f:
        f.write(public_instance_id)
        
    launch_private_instance(subnets["private"][0], private_sg, "Private-Instance", private_key)
